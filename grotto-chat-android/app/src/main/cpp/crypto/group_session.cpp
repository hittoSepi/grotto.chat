...
#define LOG_TAG "GrottoGroupSession"
...
GroupSession::GroupSession(signal_protocol_store_context* store_ctx,
    ...
    LOGD("GroupSession created with store_ctx=%p, signal_ctx=%p", store_ctx, signal_ctx);
}

GroupSession::~GroupSession() = default;

std::vector<uint8_t> GroupSession::create_session(const std::string& channel_id) {
    LOGD("GroupSession::create_session for '%s' (len=%zu), local_id='%s' (len=%zu), device_id=%d",
         channel_id.c_str(), channel_id.size(), local_id_.c_str(), local_id_.size(), local_device_id_);
    LOGD("store_ctx=%p, signal_ctx=%p", store_ctx_, signal_ctx_);
    
    if (channel_id.empty()) {
        LOGE("channel_id is empty!");
        throw std::runtime_error("channel_id is empty");
    }
    if (local_id_.empty()) {
        LOGE("local_id is empty!");
        throw std::runtime_error("local_id is empty");
    }
    ...
    LOGD("sender_key_name: group=%s, sender=%s, device=%d",
         sender_key_name.group_id, sender_key_name.sender.name, sender_key_name.sender.device_id);

    LOGD("Creating group_session_builder...");
    LOGD("store_ctx=%p, signal_ctx=%p", store_ctx_, signal_ctx_);
    group_session_builder* builder = nullptr;
    int rc = group_session_builder_create(&builder, store_ctx_, signal_ctx_);
    if (rc != SG_SUCCESS) {
        LOGE("group_session_builder_create failed: %d", rc);
        throw std::runtime_error("group_session_builder_create failed: " + std::to_string(rc));
    }
    LOGD("group_session_builder created at %p", builder);
    LOGD("Calling group_session_builder_create_session...");
    LOGD("sender_key_name: group=%s, sender=%s, device=%d", 
         sender_key_name.group_id, sender_key_name.sender.name, sender_key_name.sender.device_id);
    ...
    if (rc != SG_SUCCESS) {
        LOGE("group_session_builder_create_session failed: %d", rc);
        throw std::runtime_error("group_session_builder_create_session failed: " + std::to_string(rc));
    }
    LOGD("group_session_builder_create_session succeeded");
    ...
    LOGD("Created group session for %s, SKDM size: %zu bytes", channel_id.c_str(), result.size());
    return result;
}
    ...
    const std::vector<uint8_t>& distribution_msg)
{
    ...
    if (rc != SG_SUCCESS) {
        LOGE("Failed to deserialize SenderKeyDistributionMessage: %d", rc);
        return;
    }
    ...
    if (rc == SG_SUCCESS) {
        rc = group_session_builder_process_session(builder, &sender_key_name, skdm);
        if (rc != SG_SUCCESS) {
            LOGE("group_session_builder_process_session failed: %d", rc);
        } else {
...
std::vector<uint8_t> GroupSession::encrypt(const std::string& channel_id,
    ...
    if (rc != SG_SUCCESS) {
        ...
        throw std::runtime_error("group_cipher_create failed");
    }
    ...
    if (rc != SG_SUCCESS) {
        ...
        throw std::runtime_error("group_cipher_encrypt failed");
    }
    ...
    LOGD("Encrypted group message for %s: %zu bytes -> %zu bytes", 
         channel_id.c_str(), plaintext.size(), result.size());
    return result;
}
