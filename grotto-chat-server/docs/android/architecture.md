...
Grotto-serveri ja desktop-client ovat C++20. Signal Protocol -sessiot, Opus-codec ja
...
```
grotto-native/                    ← Jaettu C++ moduuli
├── CMakeLists.txt
├── crypto/
...
│       │   ├── GrottoApp.kt                  # Application, Hilt entry
...
│       │   │   ├── remote/
│       │   │   │   ├── GrottoSocket.kt        # TLS TCP framing
...
│       │   ├── ui/
│       │   │   ├── theme/
│       │   │   │   ├── Theme.kt               # Material3 + GrottoTheme
...
│       │   │
│       │   └── service/
│       │       ├── GrottoService.kt           # Foreground Service (connection)
...
│           └── → symlink/copy grotto-native/
│
├── native/                                    # Shared C++ moduuli
│   ├── CMakeLists.txt
│   └── → grotto-native/ (ks. yllä)
│
...
### GrottoService — Persistent Connection

```kotlin
class GrottoService : Service() {
...
|------|-----------|----------|---------|
| 1. Foreground | GrottoService, TCP keepalive | <100ms | Korkea |
...
Serveri havaitsee: user offline, offline_queue kasvaa
  ...
  → Android vastaanottaa: käynnistä GrottoService
  → GrottoService yhdistää serveriin
