...
│   └── session_test.cpp
└── deploy/
    ├── grotto-server.service           # systemd unit
...
message Hello {
  ...
  string client_version   = 2;  // "grotto-client/0.1.0"
}
...
[tls]
cert_file = "/etc/grotto/server.crt"
key_file  = "/etc/grotto/server.key"

[database]
path      = "/var/lib/grotto/grotto.db"
...
```ini
# /etc/systemd/system/grotto-server.service
[Unit]
...
User=grotto
Group=grotto
ExecStart=/usr/local/bin/grotto-server --config /etc/grotto/server.toml
Restart=on-failure
RestartSec=5
...
DOMAIN="grotto.sinundomainisi.fi"
