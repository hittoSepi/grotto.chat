...
SSL_CTX_set_verify(ctx, SSL_VERIFY_PEER, [](int ok, X509_STORE_CTX* ctx) {
    ...
    // vertaa tallennettuun ~/.config/grotto/server_cert.pin
...
- Timeout: 5 sekuntia
- User-Agent: `grotto-preview/1.0` (rehellinen)
...
```toml
# ~/.config/grotto/config.toml

[server]
host         = "grotto.example.duckdns.org"
port         = 6667
