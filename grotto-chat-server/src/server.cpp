...
{
    spdlog::info("Initializing Grotto Server...");
    ...
    try {
        ...
        try {
            auto file_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(
                "grotto-server.log", 1024 * 1024 * 5, 3);  // 5MB, 3 files
...
void Server::run() {
    ...
    spdlog::info("Starting Grotto Server v0.1.0");
...
#ifdef GROTTO_CHAT_HTTP_API_ENABLED
    ...
    if (config_.http_api_enabled) {
        ...
        http_api_server_ = std::make_unique<grotto::api::Server>(ioc_, api_config);
...
void Server::setup_http_api_routes() {
    ...
    http_api_server_->post("/api/v1/bug-reports", [this](const auto& req) {
        if (!req.has_json_body()) {
            return grotto::api::Response::bad_request("JSON body required");
        }
        
        auto body = req.json_body();
        
        if (!body.contains("description")) {
            return grotto::api::Response::bad_request("Missing 'description' field");
        }
        ...
        if (description.length() < 10) {
            return grotto::api::Response::bad_request("Description too short (min 10 chars)");
        }
...
void Server::handle_admin_command(const nlohmann::json& cmd) {
    ...
    if (command == "kick") {
        ...
        send_admin_response("status", true, {
            {"uptime", uptime},
            {"connections", conns},
            {"version", std::string(grotto::kProjectVersion)}
        });
