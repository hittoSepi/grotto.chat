...
namespace grotto {
	...
	ServerConfig ConfigLoader::load( const std::string &config_path ) {
		...
		if ( !grotto::file_exists( _path ) ) {
			std::cout << "Creating new config file.." << std::endl;
			grotto::create_config_file( _path );
		}
		...
		if ( data.contains( "directory" ) ) {
			...
			config.directory_url = get_optional<std::string>( dir, "url", "https://directory.grotto.dev" );
		...
		if ( data.contains( "admin" ) ) {
			...
			config.admin_tui_socket = get_optional<std::string>( admin, "tui_socket", "/run/grotto/admin.sock" );
