# Grotto Server Scripts

## install.sh

Stage-1 bootstrap for the Grotto Linux installer.
...
2. Download the matching `grotto-installer` binary
...
```bash
wget https://raw.githubusercontent.com/hittoSepi/grotto/main/grotto-server/scripts/install-legacy-build.sh
...
4. **Configures the system:**
   - Creates `grotto` user
   - Sets up directories (`/opt/grotto`, `/var/lib/grotto`)
...
```bash
# Check status
systemctl status grotto-server

# View logs
journalctl -u grotto-server -f

# Edit config
nano /opt/grotto/server.toml
...
```bash
export GROTTO_CHAT_DOMAIN="chat.example.com"
export GROTTO_CHAT_PORT="6697"
export GROTTO_CHAT_USE_LETSENCRYPT="yes"        # yes | selfsigned | skip
export GROTTO_CHAT_LE_METHOD="standalone"       # standalone | dns-cloudflare
export GROTTO_CHAT_CF_TOKEN="your-cloudflare-token"  # for DNS method
export GROTTO_CHAT_PUBLIC="yes"                 # yes | no
...
## grotto-generate-file-encryption-key
...
# Add to server.toml:
echo "file_encryption_key = \"$(./grotto-generate-file-encryption-key)\"" >> /opt/grotto/server.toml
```
